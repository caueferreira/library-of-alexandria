...
class ExpansionRepository(
    ...
    fun store(result: ExpansionResult.Success.Network) = ExpansionResult.Success.Network(local.store(result.result))

    private fun fromCache() = ExpansionResult.Success.Cache(local.list())

    private suspend fun fromNetwork() = try {
        ExpansionResult.Success.Network(remote.list())
