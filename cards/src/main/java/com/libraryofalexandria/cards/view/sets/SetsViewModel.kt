...
class SetsViewModel(
    ...
    private var _filterBy: MutableList<SetFilter> = mutableListOf()
        ...
        viewModelScope.launch {
            filterRepository.get()
                .onSuccess { filters ->
                    ...
                    _filterBy = filters.map { it.type }.toList().toMutableList()
                }
        }
    }

    fun filterBy(filter: SetFilter) {
        _filterBy.addOrRemove(filter)
        _sets.value = _load.stream().filter { _filterBy.contains(it.setFilter) }.collect(Collectors.toList())
    }
}